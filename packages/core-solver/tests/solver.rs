//! Phase-3 solver correctness: the branch-and-bound search must agree with the
//! exhaustive brute force. A deterministic xorshift RNG drives many random small
//! inventories (a property test without extra dependencies).
//!
//! - Dominance OFF: full top-N score list must equal brute force (validates the
//!   bound + heap).
//! - Dominance ON: the optimum (top-1) must equal brute force (dominance may drop
//!   strictly-worse builds at lower ranks, so only top-1 is guaranteed equal).

use core_solver::domain::{Stat, Stats, STAT_COUNT};
use core_solver::solver::brute::brute_force;
use core_solver::solver::model::Disc;
use core_solver::solver::{solve, solve_with, BuildResult, Constraint, Objective};
...
fn rand_disc(rng: &mut Rng, id: u32, slot: u8) -> Disc {
    ...
    Disc { id, set: 0, slot, main: (main_stat, main_val), subs }
}
...
fn single_candidate_per_slot_is_deterministic() {
    ...
    let res = solve(&discs, &b, &obj, &[], 3);
    assert_eq!(res.len(), 1, "only one build is possible");
    // AtkPct = 6*100fp = 0.6 -> 600 ; CR = 50 + 6*20 = 170fp = 0.17 -> 340 ;
    // CD = base 500fp = 0.5 -> 750 ; total = 1690
...
fn bnb_topn_matches_brute_without_dominance() {
    ...
    for _ in 0..400 {
        ...
        let bnb = solve_with(&inv, &b, &obj, &[], 5, false);
        let bf = brute_force(&inv, &b, &obj, &[], 5);
...
fn bnb_top1_matches_brute_with_dominance() {
    ...
    for _ in 0..400 {
        let inv = rand_inventory(&mut rng);
        let bnb = solve(&inv, &b, &obj, &[], 1);
        let bf = brute_force(&inv, &b, &obj, &[], 1);
        assert_scores_eq(&bnb, &bf);
    }
}
...
fn constraints_respected_match_brute() {
    ...
    for _ in 0..400 {
        let inv = rand_inventory(&mut rng);
        let bnb = solve_with(&inv, &b, &obj, &cons, 5, false);
        let bf = brute_force(&inv, &b, &obj, &cons, 5);
        assert_scores_eq(&bnb, &bf);
    }
}
